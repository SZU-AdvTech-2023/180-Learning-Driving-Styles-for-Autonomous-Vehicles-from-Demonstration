...
class MotionPlanner:
    def __init__(self, trajectory_len, feature_len, device, max_iterations=2, step_size=0.4, test=False):
        ...
        control_variables = th.Vector(dof=32, name="control_variables")
...
def acceleration(optim_vars, aux_vars):
    control_variables = optim_vars[0].tensor.view(-1, 32)
    lat_a = control_variables[:, :16]
    lon_a = control_variables[:, 16:]
