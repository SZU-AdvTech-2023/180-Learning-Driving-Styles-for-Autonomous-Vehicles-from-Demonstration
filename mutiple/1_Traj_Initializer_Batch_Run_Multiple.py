...
from model.LfD_Base_Multiple import *
    ...
    for task_epoch in range(1, args.task_epochs+1):
        ...
        for x_spt, x_qry in zip(train_loader, test_loader):
            ...
            for epoch in range(args.train_epochs):
                ...
                # IRL
                lat_loc = cal_quintic_multiple_spline_point(control_var[:,:16], current_state[:, 0 ,0], current_state[:, 0 ,3])
                lon_loc = cal_quintic_multiple_spline_point(control_var[:,16:], current_state[:, 0 ,1], current_state[:, 0 ,4])
            ...
            for epoch in range(args.test_epochs):
                ...
                lat_loc = cal_quintic_multiple_spline_point(control_var[:,:16], t_current_state[:,0 ,0], t_current_state[:,0 ,3])
                lon_loc = cal_quintic_multiple_spline_point(control_var[:,16:], t_current_state[:,0 ,1], t_current_state[:,0 ,4])
                traj = torch.stack([lat_loc, lon_loc],dim=-1)
