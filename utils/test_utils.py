...
def show_figure(raw_traj, before_traj, cross_traj,after_traj,expert_traj):
    # 绘制ground_truth[0, 0,:,0]
    for i in range(3):
        # 创建新的figure
        plt.figure()
        plt.plot(raw_traj[i,:,0],raw_traj[i,:,1], label='raw_traj')
        # plt.plot(before_traj[i,:,0],before_traj[i,:,1], label='before_traj')
        plt.plot(cross_traj[i,:,0],cross_traj[i,:,1], label='cross_traj')
        # plt.plot(after_traj[i,:,0],after_traj[i,:,1], label='after_traj')
        plt.plot(expert_traj[i,:,0],expert_traj[i,:,1], label='expert_traj')
...
def bicycle_model_test(control, current_state):
    dt = 0.1 # discrete time period [s]
    max_delta = 0.6 # vehicle's steering limits [rad]
    max_a = 5 # vehicle's accleration limits [m/s^2]
...
def check_dynamics(traj):
    ...
    return lon_speed, lat_speed, np.mean(np.abs(acc)), np.mean(np.abs(jerk)), np.mean(np.abs(lat_acc))

def check_speed_mean_error(traj, ref_line):
    speed = traj[:, -1]
    speed = np.clip(speed, a_min=0, a_max=None)
    speed_limit = np.max(ref_line[:, -1], axis=-1, keepdims=True)[0]
    speed_mean = np.mean(speed)
    speed_mean_error = np.mean(np.abs(speed - speed_limit))
    return speed_mean_error, speed_mean
