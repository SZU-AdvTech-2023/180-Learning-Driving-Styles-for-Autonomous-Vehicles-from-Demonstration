...
from model.LfD_Base import cal_traj_features
...
def check_feature_min_max_info(features):
    logging.info(f"lat_acc: {features[:,0].min():.4f}, {features[:,0].max():.4f}")
    logging.info(f"lon_acc: {features[:,1].min():.4f}, {features[:,1].max():.4f}")
    logging.info(f"lat_jerk: {features[:,2].min():.4f}, {features[:,2].max():.4f}")
    logging.info(f"lon_jerk: {features[:,3].min():.4f}, {features[:,3].max():.4f}")
    logging.info(f"f_k2: {features[:,4].min():.4f}, {features[:,4].max():.4f}")
    logging.info(f"speed_efficiency: {features[:,5].min():.4f}, {features[:,5].max():.4f}")
    logging.info(f"lane_mean_error: {features[:,6].min():.4f}, {features[:,6].max():.4f}")
    logging.info(f"risky_cost: {features[:,7].min():.4f}, {features[:,7].max():.4f}")
...
action_type_dict =  {'stationary':1, 'straight':2, 'straight_right':3, 
                     'straight_left':4, 'slight_right_turn':5, 'sharp_right_turn':6, 
                     'right_u_turn':7, 'slight_left_turn':8, 'sharp_left_turn':9, 
                     'left_u_turn':10, 'None':0} # left big turn
behavior_list = [2,3,4,5,6,8,9] # 有效的多风格行为
# 不同行为聚类个数不同
cluster_N = [0,
             5,5,5,5,5,
             5,0,5,5,0]
...
def data_generation():
    ...
    # process file
    task_manager = DataManager_Task(args.process_path + "traj_" +args.name, args.save_path + "task_" +args.name)
    ...
    for i,batch in enumerate(data_loader):
        # if i < 1: # train settings
        #     continue
        # if i < 1: # test settings
        #     continue

        # prepare data
        # prediction = batch[0].to(device)
        ref_line = batch[0].to(device)[:,::2,:]
        current_state = batch[1].to(device)
        ground_truth = batch[2].to(device)
        ego = batch[3].to(device)
        plan = batch[4].to(device)
        plan_traj = bicycle_model_plan2traj(plan, current_state[:,0])
        ...
        # egoism reward 自我奖励 
        # 1、舒适度 jerk
        # comfort_cost = cal_comfort_cost(ground_truth[:, 0].cpu().numpy())
        
        # # 2、效率 speed
        # efficiency_cost = cal_efficiency_cost(ground_truth[:, 0], ref_line)

        # # 3、风险 risk
        # risky_cost = cal_risky_cost(ground_truth[:, 0], ground_truth[:, 1:], current_state, ref_line)

        # # 4、横、纵向 偏移 lat/lon
        # lane_cost = cal_lane_error_cost(ground_truth[:, 0], ref_line, current_state)

        # check info
        # check_traj_info(ground_truth[:, 0], efficiency_cost, risky_cost, lane_cost)

        features = cal_traj_features(ground_truth[:, 0], ref_line, ground_truth[:, 1:], current_state)
        check_feature_min_max_info(features)
        features_np = features.cpu().numpy()
        # No
        # courtesy reward 礼貌奖励
        # 他人原始的行为分布以及受影响后的行为分布之间的差异。

        # No
        # confidence reward 自信奖励
        # 主动做出动作以减少对方反应的不确定性

        # data save
        plan_traj, ego, ref_line, current_state, ground_truth = convert2np(plan_traj, ego, ref_line, current_state, ground_truth)
        # efficiency_cost, risky_cost, lane_cost = convert2np(efficiency_cost,risky_cost,lane_cost)

        action_type = get_action_np(ego, ground_truth)
        # action_type:  torch.Size([8500, 1])
        # print(f"action_type: ",action_type.shape)
        
        # 取task计算k-mean
        """以下全是np计算"""
        behavior_N = len(action_type_dict)
        for behavior_i in range(1,behavior_N):
            behavior_idx = action_type==behavior_i
            # print("behavior_idx: ",behavior_idx.shape)
            ...
            # print("plan_behavior: ",plan_behavior.shape)
            # prediction_behavior = prediction[behavior_idx]
            plan_traj_behavior = plan_traj[behavior_idx]
            ...
            # cost feature
            # comfort_cost_behavior = comfort_cost[behavior_idx]
            # efficiency_cost_behavior = efficiency_cost[behavior_idx]
            # risky_cost_behavior = risky_cost[behavior_idx]
            # lane_cost_behavior = lane_cost[behavior_idx]
            features_behavior = features_np[behavior_idx]
            logging.info(f"behavior_{behavior_i} length: {np.sum(behavior_idx)}  percent: {np.sum(behavior_idx)*100/args.dataset_size:.1f}%")
        
            # print(f"behavior_{behavior_i} shape: ",behavior_idx.shape)
            # print("comfort_cost_behavior: ",comfort_cost_behavior.shape)
            # print("efficiency_cost_behavior: ",efficiency_cost_behavior.shape)
            # print("risky_cost_behavior: ",risky_cost_behavior.shape)
            # print("lane_cost_behavior: ",lane_cost_behavior.shape)
            # feature_behavior = np.vstack((comfort_cost_behavior,efficiency_cost_behavior,risky_cost_behavior,lane_cost_behavior)).T

            # print("feature_behavior: ",feature_behavior.shape)
            # input example
            ...
            # find N
            select_task_label, all_task_label = k_means.classify_dataset_find_N_Max(features_behavior,find_Max_N=48)
            # find All
            # all_task_label = k_means.classify_dataset(feature_behavior)
            # print(k_means.centers_)

            for style_i in range(cluster_N[behavior_i]):
                # find All
                # style_idx = all_task_label==style_i
                # find N
                style_idx = select_task_label[style_i]
                # print("style_idx: ",style_idx.shape)
                # plan_style = plan_behavior[style_idx]
                # print("plan_style: ",plan_style.shape)
                # prediction_style = prediction_behavior[style_idx]
                plan_traj_style = plan_traj_behavior[style_idx]
                ...
                if np.sum(all_style_idx)<check_N:
                    check_find = False
                if behavior_i in behavior_list:
                    task_manager.save_data(behavior_i, style_i, plan_traj_style, ref_line_style, current_state_style, ground_truth_style)
